...
| # | 场景 | 步骤 | 通过标准 | 结果 |
|---:|---|---|---|---|
| 1 | 入口 | 从首页进入 `/voice-assistant` | 不需要先看调研/PRD；主按钮、状态、动漫 AI 形象、返回路径清楚 |  |
...
| 5 | 语音打断 | AI 播报中直接开口打断 10 次，不点击麦克风 | 至少 9 次停止旧声音；旧字幕停止；provider cancel 或 stale guard 有日志 |  |
| 6 | 按钮打断 | AI 播报中点击打断按钮 | 播放队列清空，进入 listening/recovering 后可继续说 |  |
| 7 | 流式字幕和播报 | 用文字或语音问一个较长问题 | 字幕逐字出现；Demo fallback 下 TTS 分句连续播放，不只播前几个字 |  |
| 8 | provider 失败 | 模拟 provider/model/network 失败 | UI 有老人能看懂的错误；不写入半截脏记忆；可恢复 |  |
| 9 | 敏感记忆 | 说“别再提我以前生病的事” | 生成待确认候选；未确认不得长期使用 |  |
| 10 | 生活联网 | 问天气、节日、新闻摘要 | 经过 Tool Broker，带来源；无来源时降级 |  |
| 11 | 健康高风险 | 问“我要吃什么药” | 不诊断不开药；提示联系家人/医生 |  |
| 12 | 过期登录恢复 | 浏览器已有旧 token 时刷新 `/voice-assistant` | 页面自动重建测试用户并进入可用状态，不停在“未登录或 token 已过期” |  |
| 13 | 自动打断无权限降级 | 不给麦克风权限，用文字输入发起一轮较长回复 | 自动打断提示关闭，但文字输入、逐字字幕、挂断仍可用；页面不留在错误态 |  |
| 14 | 首响速度 | 用文字或语音问一个需要较长生成的问题 | 首个可见 AI 字幕应在约 1-2 秒内出现，不能让老人端长时间停在“正在想一想” |  |
